//* 
// opendoor_shadowrocket.js
// 说明: Shadowrocket 脚本 - 另一条开门接口路径（ID 不同）
// 触发条件: http-request ^https://loonjs\.ai/opendoor$
// 逻辑同 dooropen_loon.js，仅 OPEN_ID 不同
// 在Shadowrocket配置中添加:
// http-request ^https:\/\/loonjs\.ai\/(opendoor|dooropen)$ script-path=opendoor_shadowrocket.js, argument="opendoor=<id>&dooropen=<id>"
...
// notify: 统一通知格式输出（Shadowrocket 3参数版本）
//   title: 标题
//   subtitle: 子标题
//   message: 内容
...
function resolveToken() {
	...
	if (!token) {
		...
		notify('芝麻开门', '', 'access-token 未找到，请重新获取');
...
function openDoor(id) {
	...
	$httpClient.post({
		...
		if (parsed && typeof parsed.message === 'string' && parsed.message.includes('token') && !parsed.message.includes('openResult=True')) {
			...
			notify('芝麻开门', '', 'access-token失效，已清理，请重新获取');
